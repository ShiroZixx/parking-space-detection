...
class SolutionAnnotator(Annotator):
    ...
    def display_analytics(
        ...
        """
        Hiển thị bảng thống kê kiểu hiện đại: bo góc, semi-transparent, padding đẹp.
        Riêng Occupancy: đỏ; Available: xanh lá.
        """
        h, w, _ = im0.shape
        card_width = 220
        line_height = 30
        ...
        thickness = 1  # độ dày chữ nhỏ lại
        font_scale = 0.8

        # Tính toán tổng chiều cao
        total_height = padding * 2 + line_height * len(text)

        # Vị trí góc trên bên phải
        x1 = w - card_width - margin
        y1 = margin
        x2 = w - margin
        y2 = y1 + total_height
        ...
        # Vẽ rounded rectangle semi-transparent
        self.draw_rounded_rectangle(
            overlay, (x1, y1), (x2, y2), bg_color, corner_radius, -1
        )
        ...
        # Vẽ text
        y_text = y1 + padding + line_height - 8
        for label, value in text.items():
            txt = f"{label}: {value}"

            # Đổi màu chữ theo label
            if label.lower() == "occupancy":
                color = (0, 0, 255)  # đỏ (BGR)
            elif label.lower() == "available":
    ...
    def draw_rounded_rectangle(self, img, top_left, bottom_right, color, radius, thickness):
        """
        Vẽ rounded rectangle (bo góc) bằng OpenCV.
        """
        x1, y1 = top_left
        x2, y2 = bottom_right
